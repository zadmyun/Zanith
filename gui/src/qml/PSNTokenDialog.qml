...
DialogView {
    ...
    title: {
        if(expired)
            root.zt("Credentials Expired: Refresh PSN Remote Connection", "Credenciais expiradas: atualizar conexão remota PSN")
        else
            root.zt("Setup Automatic PSN Remote Connection", "Configurar conexão remota automática via PSN")
    }
    buttonText: root.zt("Setup", "Configurar")
    ...
    Item {
        ...
        GridLayout {
            ...
            Label {
                text: root.zt("Open Web Browser with copied URL", "Abrir navegador com a URL copiada")
                visible: psnurl
            }
            ...
            C.Button {
                id: copyUrl
                text: root.zt("Click to Re-Copy URL", "Clique para copiar a URL novamente")
                onClicked: {
                    openurl.selectAll()
                    openurl.copy()
                }
            ...
            Label {
                text: root.zt("Redirect URL from Web Browser", "URL de redirecionamento do navegador")
            }
            ...
            TextField {
                ...
                C.Button {
                    id: pasteUrl
                    text: root.zt("Click to Paste URL", "Clique para colar a URL")
                    anchors {
                        left: parent.right
                        verticalCenter: parent.verticalCenter
                        leftMargin: 10
        ...
        Item {
            Dialog {
                ...
                title: root.zt("Create PSN Automatic Remote Connection Token", "Criar token de conexão remota automática PSN")
                modal: true
